...
//!
//! Root-authorized freezing/thawing of an account's native (`Balances`) balance, on top of
//! `pallet-balances`' `fungible::freeze` mechanism. Unlike a currency lock or reserve, a freeze
//! reduces an account's *free* (transferable) balance while still counting it toward the
//! account's total balance and existential-deposit accounting.
//!
//! Four dispatchables, all restricted to [`Config::FreezeOrigin`]:
//! - [`Pallet::freeze_account`] freezes an account's entire balance indefinitely.
//! - [`Pallet::freeze_amount`] freezes a specific amount.
//! - [`Pallet::thaw_account`] fully lifts a freeze.
//! - [`Pallet::thaw_amount`] releases part of a freeze, leaving the rest in place.
//!
//! The [`Frozen`] storage map tracks which accounts currently have an active freeze, to prevent
//! stacking multiple freezes on top of each other.
//!
...
pub mod pallet {
	...
	/// Balance type of the currency this pallet freezes, derived from [`Config::Currency`].
	...
	pub trait Config: frame_system::Config {
		...
		/// The fungible implementation used to freeze/thaw the native balance.
	...
	#[pallet::storage]
	pub type Frozen<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, (), OptionQuery>;
